...
// PZ kategórie: faktury, objednavky, pz_ostatne — zoskupiť podľa roku nahrania
$_pzCats = ['faktury', 'objednavky', 'pz_ostatne'];
$_listed_years = ['2026', '2025', '2024', '2023', '2022', '2021'];

// $_byYear[$year][$cat] = array of files
$_byYear = [];
foreach ($_listed_years as $y) $_byYear[$y] = ['faktury'=>[], 'objednavky'=>[], 'pz_ostatne'=>[]];
$_byYear['archiv'] = ['faktury'=>[], 'objednavky'=>[], 'pz_ostatne'=>[]];

foreach ($_subory as $f) {
    $cat = $f['category'] ?? '';
    if (!in_array($cat, $_pzCats)) continue;
    if (empty($f['filename'])) continue;
    $year = !empty($f['uploaded']) ? substr($f['uploaded'], 0, 4) : '0000';
    if (in_array($year, $_listed_years)) {
        $_byYear[$year][$cat][] = $f;
    } else {
        $_byYear['archiv'][$cat][] = $f;
    }
}

function _pz_size(int $b): string { return $b>=1048576?round($b/1048576,1).' MB':($b>=1024?round($b/1024).' kB':$b.' B'); }
function _pz_esc(string $s): string { return htmlspecialchars($s, ENT_QUOTES, 'UTF-8'); }

function _pz_items(array $files, string $cat): void {
    if (empty($files)) {
        echo '<div class="pdf-empty"><svg viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6zm4 18H6V4h7v5h5v11z"/></svg>Zatiaľ žiadne dokumenty</div>';
        return;
    }
    foreach ($files as $f) {
        $href = 'pdf/' . _pz_esc($cat) . '/' . _pz_esc($f['filename']);
        $name = _pz_esc($f['label'] ?? $f['filename']);
        $meta = !empty($f['size']) ? _pz_size((int)$f['size']) : 'PDF dokument';
        echo '<a class="pdf-item" href="' . $href . '" target="_blank">'
           ...
           . '<div class="pdf-info"><div class="pdf-name">' . $name . '</div><div class="pdf-size">' . $meta . '</div></div>'
           . '<div class="pdf-download"><svg viewBox="0 0 24 24"><path d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z"/></svg></div>'
           . '</a>';
    }
}

$_crz_link = '<a class="ext-link" href="https://www.crz.gov.sk/" target="_blank" rel="noopener">'
    . '<div class="ext-icon"><svg viewBox="0 0 24 24"><path d="M19 19H5V5h7V3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7h-2v7zM14 3v2h3.59l-9.83 9.83 1.41 1.41L19 6.41V10h2V3h-7z"/></svg></div>'
    . '<div class="ext-info"><div class="ext-name">Zmluvy – Centrálny register</div><div class="ext-desc">Otvoriť na crz.gov.sk</div></div>'
    . '</a>';
?>
<!DOCTYPE html>
<html lang="sk">
<head>
  ...
  <title>Povinné zverejňovanie – CSS Studienka</title>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* ── HLAVNÝ OBSAH ── */
    .main-wrap { max-width: 1000px; margin: 3rem auto 5rem; padding: 0 2rem; }

    /* ── PREPÍNAČ ROKOV ── */
    .year-tabs { display: flex; gap: .5rem; flex-wrap: wrap; margin-bottom: 2.5rem; background: var(--white); padding: 1rem 1.25rem; border-radius: var(--radius); box-shadow: var(--shadow); align-items: center; }
    .year-tabs-label { font-size: .78rem; font-weight: 700; letter-spacing: .07em; text-transform: uppercase; color: var(--gray-md); margin-right: .5rem; white-space: nowrap; }
    .year-btn { padding: .45rem 1.1rem; border-radius: 2rem; font-family: inherit; font-size: .88rem; font-weight: 600; cursor: pointer; border: 2px solid var(--gray-lt); background: transparent; color: var(--gray-md); transition: all .2s; }
    .year-btn:hover { border-color: var(--teal); color: var(--teal); }
    .year-btn.active { background: var(--navy); border-color: var(--navy); color: var(--white); }

    /* ── PANEL ROKA ── */
    .year-panel { display: none; }
    .year-panel.active { display: block; }
    .year-panel-title { font-family: 'Playfair Display', serif; font-size: 1.4rem; color: var(--navy); margin-bottom: 1.75rem; display: flex; align-items: center; gap: .75rem; }
    .year-panel-title::after { content: ''; flex: 1; height: 2px; background: var(--gray-lt); border-radius: 2px; }

    /* ── GRID KATEGÓRIÍ ── */
    .categories-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 1.5rem; }
    .category-box { background: var(--white); border-radius: var(--radius); box-shadow: var(--shadow); overflow: hidden; }
    .category-header { display: flex; align-items: center; gap: .75rem; padding: .9rem 1.25rem; background: var(--navy); color: var(--white); }
    .category-header svg { width: 20px; height: 20px; fill: var(--gold); flex-shrink: 0; }
    .category-header h3 { font-family: 'Playfair Display', serif; font-size: 1rem; font-weight: 600; }
    .category-body { padding: .75rem; }

    /* ── PDF POLOŽKY ── */
    .pdf-item { display: flex; align-items: center; gap: .85rem; padding: .7rem .85rem; border-radius: 6px; transition: background .15s, transform .15s; cursor: pointer; }
    .pdf-item:hover { background: var(--gray-lt); transform: translateX(3px); }
    .pdf-icon { width: 36px; height: 36px; background: #fdecea; border-radius: 7px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
    .pdf-icon svg { width: 20px; height: 20px; fill: #c0392b; }
    .pdf-info { flex: 1; min-width: 0; }
    .pdf-info .pdf-name { font-size: .88rem; font-weight: 600; color: var(--navy); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .pdf-info .pdf-size { font-size: .75rem; color: var(--gray-md); }
    .pdf-download { color: var(--teal); flex-shrink: 0; opacity: .6; transition: opacity .2s; }
    .pdf-download svg { width: 16px; height: 16px; fill: var(--teal); }
    .pdf-item:hover .pdf-download { opacity: 1; }

    /* prázdny stav */
    .pdf-empty { text-align: center; padding: 1.5rem 1rem; color: var(--gray-md); font-size: .88rem; }
    .pdf-empty svg { width: 32px; height: 32px; fill: #ccc; display: block; margin: 0 auto .5rem; }

    /* externý odkaz (zmluvy CRZ) */
    .ext-link { display: flex; align-items: center; gap: .85rem; padding: .7rem .85rem; border-radius: 6px; transition: background .15s; color: var(--navy); }
    .ext-link:hover { background: var(--gray-lt); }
    .ext-icon { width: 36px; height: 36px; background: #e8f4f2; border-radius: 7px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
    .ext-icon svg { width: 20px; height: 20px; fill: var(--teal); }
    .ext-info .ext-name { font-size: .88rem; font-weight: 600; color: var(--navy); }
    .ext-info .ext-desc { font-size: .75rem; color: var(--gray-md); }
...
<header>
  ...
  <div class="header-main">
    ...
    <nav>
      ...
      <a href="povinnezverejnovanie.php" class="active">Povinné zverejňovanie</a>
...
<div class="page-hero">
  <div class="page-hero-inner">
    ...
    <p>Dokumenty zverejňované v súlade so zákonom č. 211/2000 Z. z. o slobodnom prístupe k informáciám</p>
  </div>
</div>
...
<div class="main-wrap">
  ...
  <!-- Prepínač rokov -->
  <div class="year-tabs">
    <span class="year-tabs-label">Rok:</span>
    <button class="year-btn active" onclick="switchYear('2026', this)">2026</button>
    ...
    <button class="year-btn" onclick="switchYear('2022', this)">2022</button>
    <button class="year-btn" onclick="switchYear('2021', this)">2021</button>
    <button class="year-btn" onclick="switchYear('archiv', this)">Archív</button>
  </div>

<?php
// Generovanie panelov pre každý rok
$panelDefs = [
    '2026' => 'Rok 2026',
    '2025' => 'Rok 2025',
    '2024' => 'Rok 2024',
    ...
    '2022' => 'Rok 2022',
    '2021' => 'Rok 2021',
    'archiv' => 'Archív (pred 2021)',
];
foreach ($panelDefs as $yr => $title):
    $isFirst = ($yr === '2026');
?>
  <div class="year-panel<?= $isFirst ? ' active' : '' ?>" id="panel-<?= $yr ?>">
    <div class="year-panel-title"><?= $title ?></div>
    <div class="categories-grid">
      ...
      <div class="category-box">
        ...
        <div class="category-body">
          <?php _pz_items($_byYear[$yr]['faktury'], 'faktury'); ?>
        </div>
      </div>
      ...
      <div class="category-box">
        ...
        <div class="category-body">
          <?php _pz_items($_byYear[$yr]['objednavky'], 'objednavky'); ?>
        </div>
      </div>

      <!-- Zmluvy — vždy externý odkaz na CRZ -->
      <div class="category-box">
        <div class="category-header">
          <svg viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6zm-1 7V3.5L18.5 9H13zM8 13h8v1H8zm0 3h8v1H8zm0-6h3v1H8z"/></svg>
          <h3>Zmluvy</h3>
        </div>
        <div class="category-body">
          <?= $_crz_link ?>
      ...
      <div class="category-box">
        ...
        <div class="category-body">
          <?php _pz_items($_byYear[$yr]['pz_ostatne'], 'pz_ostatne'); ?>
        </div>
      </div>
...
function switchYear(yr, btn) {
  ...
  var panel = document.getElementById('panel-' + yr);
