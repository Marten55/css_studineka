<?php
// Načítanie aktualít z JSON
$_dataDir = is_dir(__DIR__ . '/data') ? __DIR__ . '/data' : __DIR__ . '/studienka_admin_final/data';
$_aktFile = $_dataDir . '/aktuality.json';
$_aktuality = file_exists($_aktFile) ? (json_decode(file_get_contents($_aktFile), true) ?: []) : [];

// Len publikované, zoradené od najnovšieho
$_aktuality = array_values(array_filter($_aktuality, fn($a) => $a['published'] ?? true));
usort($_aktuality, fn($a, $b) => strcmp($b['date'] ?? '', $a['date'] ?? ''));

// Slovenské názvy mesiacov
$_mesace = ['', 'januára', 'februára', 'marca', 'apríla', 'mája', 'júna',
            'júla', 'augusta', 'septembra', 'októbra', 'novembra', 'decembra'];
function _a_date(string $d, array $m): string {
    $p = explode('-', $d);
    if (count($p) !== 3) return htmlspecialchars($d);
    return (int)$p[2] . '. ' . ($m[(int)$p[1]] ?? '') . ' ' . $p[0];
}
...
$_cats = [
    ...
    'oznamy'    => 'Oznamy',
    'projekty'  => 'Projekty',
    'spravy'    => 'Výročné správy',
];

// Archív — skupiny podľa roka/mesiaca
$_archive = [];
foreach ($_aktuality as $_a) {
    $p = explode('-', $_a['date'] ?? '');
    if (count($p) >= 2) {
        $_archive[$p[0]][$p[1]] = ($_archive[$p[0]][$p[1]] ?? 0) + 1;
    }
}
krsort($_archive);

$_mesaceNom = ['', 'Január', 'Február', 'Marec', 'Apríl', 'Máj', 'Jún',
...
<head>
  ...
  <title>Aktuality – CSS Studienka</title>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* ── HLAVNÝ OBSAH ── */
    .main-wrap { max-width: 1100px; margin: 3rem auto 5rem; padding: 0 2rem; display: grid; grid-template-columns: 1fr 320px; gap: 2rem; align-items: start; }
    .quote-banner { grid-column: 1 / -1; }
    .articles { grid-column: 1; display: flex; flex-direction: column; gap: 2rem; }

    /* ── KARTIČKA ČLÁNKU ── */
    .article-card { background: var(--white); border-radius: var(--radius); box-shadow: var(--shadow); overflow: hidden; display: grid; grid-template-columns: 220px 1fr; transition: transform .25s, box-shadow .25s; }
    .article-card:hover { transform: translateY(-3px); box-shadow: 0 10px 36px rgba(26,46,68,.16); }
    .article-img { position: relative; overflow: hidden; background: var(--gray-lt); min-height: 200px; }
    .article-img-placeholder { width: 100%; height: 100%; min-height: 200px; display: flex; align-items: center; justify-content: center; background: linear-gradient(135deg, #3d5fad, #6b8fd6); }
    .article-img-placeholder svg { width: 48px; height: 48px; fill: rgba(255,255,255,.3); }
    .article-date-badge { position: absolute; bottom: 0; left: 0; right: 0; background: linear-gradient(transparent, rgba(26,46,68,.75)); color: rgba(255,255,255,.9); font-size: .75rem; padding: 1.5rem .85rem .5rem; letter-spacing: .02em; }
    .article-body { padding: 1.5rem 1.75rem; display: flex; flex-direction: column; justify-content: space-between; gap: 1rem; }
    .article-tag { display: inline-block; background: var(--gray-lt); color: var(--teal); font-size: .72rem; font-weight: 700; letter-spacing: .08em; text-transform: uppercase; padding: .25rem .75rem; border-radius: 2rem; margin-bottom: .25rem; }
    .article-tag.gold { background: #fdf3db; color: #a07820; }
    .article-body h2 { font-family: 'Playfair Display', serif; font-size: 1.2rem; font-weight: 700; color: var(--navy); line-height: 1.35; margin-bottom: .4rem; }
    .article-body p { font-size: .93rem; color: #555; line-height: 1.65; }
    .article-meta { display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: .5rem; margin-top: .5rem; padding-top: .75rem; border-top: 1px solid var(--gray-lt); }
    .article-meta-date { font-size: .8rem; color: var(--gray-md); display: flex; align-items: center; gap: .35rem; }
    .article-meta-date svg { width: 14px; height: 14px; fill: var(--gray-md); }

    /* ── PRÁZDNY STAV ── */
    .empty-state { text-align: center; padding: 4rem 2rem; background: var(--white); border-radius: var(--radius); box-shadow: var(--shadow); }
    .empty-state svg { width: 48px; height: 48px; fill: #ccc; margin: 0 auto 1rem; display: block; }
    .empty-state p { color: var(--gray-md); font-size: 1rem; }

    /* ── SIDEBAR ── */
    .sidebar-wrap { display: flex; flex-direction: column; gap: 1.5rem; position: sticky; top: 85px; }
    .sidebar-box { background: var(--white); border-radius: var(--radius); box-shadow: var(--shadow); overflow: hidden; }
    .sidebar-box-title { background: var(--navy); color: var(--white); font-size: .78rem; font-weight: 700; letter-spacing: .08em; text-transform: uppercase; padding: .85rem 1.25rem; display: flex; align-items: center; gap: .5rem; }
    .sidebar-box-title svg { width: 16px; height: 16px; fill: var(--gold); }
    .sidebar-box-body { padding: 1.25rem; }
    .archive-body { padding: 0 !important; max-height: 380px; overflow-y: auto; }
    .archive-year { border-bottom: 1px solid var(--gray-lt); }
    .archive-year:last-child { border-bottom: none; }
    .archive-year-btn { width: 100%; display: flex; align-items: center; justify-content: space-between; padding: .75rem 1.25rem; background: transparent; border: none; cursor: pointer; font-family: inherit; font-size: .9rem; font-weight: 700; color: var(--navy); transition: background .15s; }
    .archive-year-btn:hover { background: var(--gray-lt); }
    .archive-year-btn.open { color: var(--teal); }
    .archive-year-btn svg { width: 20px; height: 20px; fill: var(--gray-md); transition: transform .25s; flex-shrink: 0; }
    .archive-year-btn.open svg { transform: rotate(180deg); fill: var(--teal); }
    .archive-months { list-style: none; max-height: 0; overflow: hidden; transition: max-height .3s ease; background: var(--gray-lt); }
    .archive-months.open { max-height: 600px; }
    .archive-months li { border-bottom: 1px solid rgba(0,0,0,.05); }
    .archive-months li a { display: flex; align-items: center; justify-content: space-between; padding: .55rem 1.5rem; font-size: .86rem; color: #555; transition: color .15s, padding-left .15s; }
    .archive-months li a:hover { color: var(--teal); padding-left: 1.85rem; }
    .archive-months li a span { background: var(--white); color: var(--gray-md); font-size: .73rem; font-weight: 700; padding: .15rem .5rem; border-radius: 2rem; }
    .tag-cloud { display: flex; flex-wrap: wrap; gap: .5rem; }
    .tag-cloud a { background: var(--gray-lt); color: var(--navy); font-size: .8rem; font-weight: 500; padding: .3rem .85rem; border-radius: 2rem; transition: background .2s, color .2s; border: 1.5px solid transparent; }
    .tag-cloud a:hover { background: var(--teal); color: var(--white); }
    .quote-box { background: linear-gradient(135deg, #13316E 0%, #2B57BB 55%, #3d5fad 100%); color: var(--white); border-radius: var(--radius); padding: 1.75rem 1.5rem; position: relative; overflow: hidden; }
    .quote-box::before { content: '\201C'; position: absolute; top: -10px; left: 10px; font-family: 'Playfair Display', serif; font-size: 7rem; color: rgba(255,255,255,.08); line-height: 1; }
    .quote-box blockquote { font-family: 'Playfair Display', serif; font-size: 1.05rem; font-style: italic; line-height: 1.6; color: rgba(255,255,255,.9); position: relative; }
    .quote-box cite { display: block; margin-top: 1rem; font-size: .8rem; color: var(--gold); font-style: normal; font-weight: 600; letter-spacing: .04em; }
    .quote-banner { grid-column: 1 / -1; background: linear-gradient(135deg, #13316E 0%, #2B57BB 55%, #3d5fad 100%); border-radius: var(--radius); padding: 1.5rem 2rem; display: flex; align-items: center; gap: 1.25rem; color: var(--white); box-shadow: var(--shadow); }
    .quote-banner svg { width: 32px; height: 32px; fill: var(--gold); flex-shrink: 0; }
    .quote-banner p { font-family: 'Playfair Display', serif; font-size: 1.05rem; font-style: italic; color: rgba(255,255,255,.9); flex: 1; }
    .quote-banner span { font-size: .8rem; color: var(--gold); font-weight: 600; white-space: nowrap; }
    ...
    @media (max-width: 900px) {
      ...
      .quote-banner { grid-column: 1; flex-direction: column; text-align: center; }
      .articles { grid-column: 1; }
      .sidebar-wrap { position: static; }
      .article-card { grid-template-columns: 180px 1fr; }
    }
    @media (max-width: 1024px) {
      nav { display: none; } .hamburger { display: flex; } .header-main { padding: .85rem 1.25rem; }
      .main-wrap { padding: 0 1.25rem; margin: 2rem auto 3.5rem; }
      .article-card { grid-template-columns: 1fr; }
      .article-img { min-height: 180px; }
      .footer-main { grid-template-columns: 1fr 1fr; } .footer-brand { grid-column: 1 / -1; }
    }
    @media (max-width: 640px) { .footer-main { grid-template-columns: 1fr; gap: 2rem; padding: 2.5rem 1.5rem 1.5rem; } }
  </style>
...
<header>
  ...
  <div class="header-main">
    ...
    <nav>
      ...
      <div class="nav-item">
        <a href="aktuality.php" class="active">Aktuality</a>
        <div class="dropdown">
          <a href="aktuality.php" class="active">Všetky aktuality</a>
...
<div class="page-hero">
  <div class="page-hero-inner">
    <h2>Aktuality</h2>
    <p>Novinky, udalosti a informácie z nášho centra</p>
  </div>
</div>
...
<div class="main-wrap">
  ...
  <div class="quote-banner">
    ...
    <p>„Staroba nie je choroba — je to privilégium tých, ktorí žili naplno."</p>
    <span>— Motto nášho centra</span>
  </div>

  <!-- Články -->
  <div class="articles">
<?php if (empty($_aktuality)): ?>
    <div class="empty-state">
      <svg viewBox="0 0 24 24"><path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2z"/></svg>
      <p>Momentálne nie sú žiadne aktuality.</p>
    </div>
<?php else: foreach ($_aktuality as $_a):
  $_catKey = $_a['category'] ?? 'oznamy';
  $_catLabel = $_cats[$_catKey] ?? ucfirst($_catKey);
  $_isOz = ($_catKey === 'oz');
  $_dateStr = _a_date($_a['date'] ?? '', $_mesace);
?>
    <article class="article-card">
      <div class="article-img">
        ...
        <div class="article-date-badge"><?= htmlspecialchars($_dateStr) ?></div>
      </div>
      <div class="article-body">
        <div>
          <span class="article-tag<?= $_isOz ? ' gold' : '' ?>"><?= htmlspecialchars($_catLabel) ?></span>
          <h2><?= htmlspecialchars($_a['title'] ?? '') ?></h2>
          <p><?= nl2br(htmlspecialchars($_a['perex'] ?? '')) ?></p>
        </div>
...
<?php endforeach; endif; ?>
  ...
  <aside class="sidebar-wrap">
    ...
    <div class="sidebar-box">
      ...
      <div class="sidebar-box-body archive-body">
<?php if (empty($_archive)): ?>
        <div style="padding:1rem;text-align:center;color:var(--gray-md);font-size:.85rem;">Zatiaľ žiadne záznamy.</div>
<?php else:
  $firstYear = true;
  foreach ($_archive as $year => $months):
    krsort($months);
    $totalYear = array_sum($months);
?>
        <div class="archive-year">
          <button class="archive-year-btn<?= $firstYear ? ' open' : '' ?>" onclick="toggleYear(this)">
            <span><?= htmlspecialchars((string)$year) ?> <small style="font-weight:400;font-size:.78rem;color:var(--gray-md)">(<?= $totalYear ?>)</small></span>
            <svg viewBox="0 0 24 24"><path d="M7 10l5 5 5-5z"/></svg>
          </button>
          <ul class="archive-months<?= $firstYear ? ' open' : '' ?>">
<?php foreach ($months as $mon => $cnt): ?>
            <li><a href="#"><?= htmlspecialchars($_mesaceNom[(int)$mon] ?? $mon) ?> <span><?= $cnt ?></span></a></li>
<?php endforeach; ?>
          </ul>
        </div>
<?php $firstYear = false; endforeach; endif; ?>
      </div>
    </div>
    ...
    <div class="sidebar-box">
      ...
      <div class="sidebar-box-body">
        <div class="tag-cloud">
<?php foreach ($_cats as $k => $v): ?>
          <a href="#"><?= htmlspecialchars($v) ?></a>
<?php endforeach; ?>
        </div>
      </div>
    </div>
    ...
    <div class="quote-box">
      <blockquote>
        „Každý deň je novou príležitosťou nájsť radosť v malých veciach."
        <cite>— Pre našich Prijímateľov</cite>
      </blockquote>
    </div>
